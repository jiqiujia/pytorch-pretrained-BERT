...
class BertForLSTMPointer(BertPreTrainedModel):
    ...
    def forward(self, input_ids, input_mask, segment_ids, tgt_ids, tgt_mask, ext_input_ids, ext_tgt_ids,
        ...
        decoder_hidden = (torch.randn((self.num_lstm_layers, batch_size, self.hidden_size)).to(device),
                          torch.randn((self.num_lstm_layers, batch_size, self.hidden_size)).to(device))
        ...
        vocab_dist = torch.scatter_add(vocab_dist, 2, ext_input_ids.unsqueeze(1).expand_as(seq_logits), seq_logits)

...
def get_word_pred_labels(segment_ids, logits, word_cnt, ner: bool):
    word_idx = 0
    word_logits = np.zeros((word_cnt, logits.shape[1]))
    for i in range(1, logits.shape[0]):
        ...
        if word_idx >= word_cnt:
            break
    if ner:
        return np.argmax(word_logits, axis=1)
    else:
...
def main():
    ...
    with io.open(os.path.join(args.vocab_dir, 'tgt_vocab.txt'), encoding='utf-8') as fin:
        for i, line in enumerate(fin.readlines()):
            ...
            ner_id_type[i] = line.strip()
    state_dict = torch.load(os.path.join(args.output_dir, WEIGHTS_NAME), map_location=device)
    model = BertForMTTokenClassification.from_pretrained(args.output_dir,
    ...
    for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            logits, ner_logits = model(input_ids, segment_ids, input_mask)
        ...
        for i in range(logits.shape[0]):
            ...
            word_pred_labels = get_word_pred_labels(segment_ids[i], logits[i], len(example.words), False)
            word_pred_ner_labels = get_word_pred_labels(segment_ids[i], ner_logits[i], len(example.words), True)
            ...
            pred_ner_types = [ner_id_type[t] for t in word_pred_ner_labels]
            pred_ners.append(pred_ner_types)
    ...
    with io.open('pred_res.txt', 'w+', encoding='utf-8') as fout:
        for example, pred_text, pred_ner in zip(eval_examples, pred_texts, pred_ners):
            # fout.write('%s\t%s\t%s%s\n' % (' '.join(example.words), ''.join([str(lb) for lb in example.labels]), pred_text, pred_ner))
            tmp = ' '.join(word + "/" + ner for word, ner in zip(example.words, pred_ner))
            fout.write(tmp + '\t' + pred_text + '\n')
